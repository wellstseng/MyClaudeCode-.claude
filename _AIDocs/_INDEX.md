...
| # | 文件名稱 | 說明 | keywords |
|---|---------|------|----------|
| 1 | Architecture.md | 系統架構總覽：原子記憶 V2.15 + Workflow Guardian + Wisdom Engine + hooks | 架構, hooks, skill, rules, 事件驅動, wisdom engine, 規則模組, guardian |
...
基於 Claude Code hooks 事件驅動的工作流監督系統，搭配雙 LLM（Claude + Ollama qwen3/qwen3.5）原子記憶管理跨 session 知識。V2.15：Dual-Backend、Wisdom Engine、衝突偵測、rules/ 模組化、Fix Escalation、Failures 自動化、Token Diet。
