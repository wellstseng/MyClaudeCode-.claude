...
export async function performWindows(params) {
...
async function findWindow(wins, title, pid) {
    for (const w of wins) {
        try {
            if (pid != null) {
                const wPid = w.processId;
                if (wPid === pid)
                    return w;
            }
            if (title) {
                const wTitle = await w.getTitle();
                if (wTitle.toLowerCase().includes(title.toLowerCase()))
                    return w;
            }
        }
        catch { /* skip */ }
