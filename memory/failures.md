...
- [臨] regex 掃原始碼抽 attribute/宣告時未濾註解 → 死碼被誤判為活型別（golden 基準沙化，差點把兩個註解掉的模組當「遺漏」補進註冊表）→ 正確做法：判定死碼要檢三形態 `#if false`、整檔/整段註解、csproj 排除；最終以編譯器驗證（CS0246 找不到型別＝註解死碼的可靠訊號），regex 只當初篩（根因: 源碼文字層面看不出編譯可見性，TSLG 142 模組抽取實战 2026-07-28）
