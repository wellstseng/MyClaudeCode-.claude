...
│   ├── _atom_index.json                            ← V5 JSON SoT（<!-- atom-breakdown -->20 atoms：core 13 + feedback 5 + 失敗模式 2 + local 0〔〕<!-- /atom-breakdown -->）
...
`memory/_atom_index.json` 為唯一機器源（<!-- atom-total -->20<!-- /atom-total --> atoms）。`_ATOM_INDEX.md` 改為自動生成的人類可讀 mirror，僅 fallback parser 使用。
...
全域 ~<!-- atom-total -->20<!-- /atom-total --> atoms 規模用 Vector Service 是殺雞用牛刀。V5 引入 in-memory BM25（~80 行手刻於 `wg_atoms.py`）：
