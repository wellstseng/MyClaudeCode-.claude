...
│   ├── _atom_index.json                            ← V5 JSON SoT（<!-- atom-breakdown -->97 atoms：core 45 + feedback 10 + 失敗模式 2 + local 40〔World4/Tools10/MemDev20/OS3/Continuity2/Vision1〕<!-- /atom-breakdown -->）
...
`memory/_atom_index.json` 為唯一機器源（<!-- atom-total -->97<!-- /atom-total --> atoms）。`_ATOM_INDEX.md` 改為自動生成的人類可讀 mirror，僅 fallback parser 使用。
...
全域 ~<!-- atom-total -->97<!-- /atom-total --> atoms 規模用 Vector Service 是殺雞用牛刀。V5 引入 in-memory BM25（~80 行手刻於 `wg_atoms.py`）：
