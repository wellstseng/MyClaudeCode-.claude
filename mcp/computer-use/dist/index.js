...
// ── Phase 1: 基礎操控 ───────────────────────────────────────────────────────
server.tool("computer_screenshot", "截取螢幕畫面。可指定視窗標題或區域。回傳圖片。注意：截圖可能包含敏感資訊，請勿在回覆中描述密碼等內容。", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `截圖失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
server.tool("computer_click", "滑鼠點擊指定座標。支援左/右/中鍵、雙擊、組合鍵、長按。座標使用截圖空間（與截圖回傳的尺寸一致），系統會自動換算為螢幕座標，不需手動換算。", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `點擊失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
server.tool("computer_type", "鍵盤輸入文字或按鍵組合。text 逐字輸入，keys 同時按下（組合鍵）。二擇一。", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `輸入失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
// ── Phase 2: 進階操控 ───────────────────────────────────────────────────────
server.tool("computer_scroll", "滾輪操作。在指定座標位置滾動。座標使用截圖空間，系統自動換算。", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `滾動失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
server.tool("computer_cursor", "游標操作：移動（move）、取得位置（position）、拖曳（drag）。座標使用截圖空間，系統自動換算為螢幕座標，不需手動換算。", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `游標操作失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
server.tool("computer_windows", "視窗管理：列出（list）、聚焦（focus）、最小化（minimize）、最大化（maximize）、關閉（close）。", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `視窗操作失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
server.tool("computer_screen_info", "查詢螢幕資訊：解析度、DPI、游標位置。", {}, async () => {
    try {
        const result = await getScreenInfo();
        return { content: [{ type: "text", text: JSON.stringify(result) }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `查詢失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
// ── Phase 3: 歷程與偵測 ─────────────────────────────────────────────────────
server.tool("computer_history", "查詢操作歷程。可取得最近 N 步操作摘要或匯出 Markdown 報告。", {
    ...
    try {
        ...
        if (action === "clear") {
            await clearHistory();
            resetStuckDetection();
            return { content: [{ type: "text", text: "歷程已清除" }] };
        }
        if (action === "export") {
            return { content: [{ type: "text", text: exportMarkdownReport() }] };
        }
        const entries = getRecentHistory(params.count ?? 10);
        return { content: [{ type: "text", text: JSON.stringify(entries, null, 2) }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `歷程查詢失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
// ── Phase 4: 測試框架 ───────────────────────────────────────────────────────
server.tool("computer_test", "測試任務管理：建立/啟動/推進/中止測試任務，查看模板和報告。", {
    action: z.enum(["templates", "start", "status", "step", "stop", "report", "list"]).describe("操作：templates=列出模板, start=建立並啟動, status=查狀態, step=完成當前步驟, stop=中止, report=產生報告, list=列出所有任務"),
    ...
    try {
        switch (params.action) {
            case "templates": {
                ...
                return { content: [{ type: "text", text: JSON.stringify(templates.map(t => ({ name: t.name, description: t.description, steps: t.steps.length })), null, 2) }] };
            }
            case "start": {
                if (!params.template)
                    throw new Error("start 需要 template 參數");
                const tmpl = await loadTemplate(params.template);
                if (!tmpl)
                    throw new Error(`模板不存在: ${params.template}`);
            ...
            case "status": {
                if (!params.taskId)
                    throw new Error("status 需要 taskId");
                const task = getTask(params.taskId);
                if (!task)
                    throw new Error(`任務不存在: ${params.taskId}`);
            ...
            case "step": {
                if (!params.taskId)
                    throw new Error("step 需要 taskId");
            ...
            case "stop": {
                if (!params.taskId)
                    throw new Error("stop 需要 taskId");
                const task = abortTask(params.taskId);
                return { content: [{ type: "text", text: generateSummary(task) }] };
            }
            case "report": {
                if (!params.taskId)
                    throw new Error("report 需要 taskId");
                const task = getTask(params.taskId);
                if (!task)
                    throw new Error(`任務不存在: ${params.taskId}`);
                return { content: [{ type: "text", text: generateReport(task) }] };
            }
            case "list": {
        ...
        return { content: [{ type: "text", text: `測試操作失敗：${err instanceof Error ? err.message : String(err)}` }], isError: true };
    }
});
// ── 啟動 ─────────────────────────────────────────────────────────────────────
async function main() {
