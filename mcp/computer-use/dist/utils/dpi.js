...
export function getDisplayScale() {
    ...
    try {
        if (process.platform === "darwin") {
            ...
            const out = execSync("system_profiler SPDisplaysDataType -json 2>/dev/null", { encoding: "utf8", timeout: 5000 });
            const data = JSON.parse(out);
            const displays = data?.SPDisplaysDataType?.[0]?.spdisplays_ndrvs;
            if (displays?.[0]) {
                const res = displays[0]["_spdisplays_resolution"];
        ...
        if (process.platform === "win32") {
            ...
            const out = execSync('powershell -c "(Get-ItemProperty \'HKCU:\\Control Panel\\Desktop\\WindowMetrics\').AppliedDPI"', { encoding: "utf8", timeout: 5000 }).trim();
 ...
 */
export function logicalToPixel(x, y) {
 ...
 */
export function pixelToLogical(x, y) {
    const s = getDisplayScale();
    return { x: Math.round(x / s), y: Math.round(y / s) };
}
/**
