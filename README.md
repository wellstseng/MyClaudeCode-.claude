...
| 子系統 | 說明 |
|--------|------|
| **Dual-Backend Ollama** | 多 backend 自動切換：primary (遠端 GPU) → fallback (本地)。三階段退避：正常→短DIE (60s)→長DIE (等到下個 6h 時段)。長DIE 觸發使用者確認（停用/保持）。靜態旗標 `enabled: false` 永久跳過 |
...
| **Response Capture** | SessionEnd 全量 + Stop hook 逐輪增量萃取（byte_offset + cooldown 120s），可操作性標準 + 6 類型 + 150 chars |
| **Cross-Session** | SessionEnd 向量比對 knowledge_queue，2+ sessions 自動晉升，4+ sessions 建議晉升。lazy search 預篩（word_overlap ≥ 0.30） |
| **Self-Iteration** | 3 條核心原則（品質函數 + 證據門檻 + 震盪偵測）+ 定期檢閱 |
| **Wisdom Engine** | 情境分類器（硬規則）+ 反思引擎（準確率統計 + 盲點偵測） |
| **Project-Aliases** | MEMORY.md 別名定義 → 跨專案身份辨識，非 CWD 專案也能注入 |
| **Related-Edge Spreading** | 沿 Related 欄位 BFS depth=1 帶出語意相關 atoms |
| **ACT-R Activation** | 時間加權排序 B_i=ln(Σt_k^{-0.5})，近期高頻 atom 優先注入 |
| **Blind-Spot Reporter** | 三重空判斷 → 注入盲點提醒，不假裝全知 |
| **Read Tracking** | PostToolUse 攔截 Read/Bash，記錄閱讀檔案+版控查詢，寫入 episodic 閱讀軌跡（壓縮摘要格式） |
| **Staging Area** | `projects/{slug}/memory/_staging/` 專案層暫存區，續接 prompt 等臨時檔案用完即清 |
| **Fix Escalation Protocol** | 同一問題修正第 2 次起強制啟動 6 Agent 精確修正會議。Guardian hook 自動偵測 retry≥2 注入 `[FixEscalation]` 信號 → `/fix-escalation` skill。連續 3 次未解決強制暫停 |
| **Failures 自動化** | UserPromptSubmit 偵測失敗關鍵字 → detached extract-worker 萃取失敗模式 → 三維路由自動寫入對應 failure atom |
| **Token Diet** | 注入前 strip 9 種 metadata + 行動/演化日誌；SessionEnd 跳已萃取段；cross-session lazy search 預篩。省 ~1550 tok/session |
