...
- Related: cognitive-patterns, feedback-rigor-standards, 原子記憶審查總結-好機制被小故障卡死非過重-拔前先實證, write-raw-對未列舉-source-靜默回-okfalse-不-raise呼叫端必檢查回傳值, aec-b欄如實載明hook標記退避語不以自評覆蓋, activation負值不是負相關-act-r對數尺度天然跨零-注入噪音修門檻與顯示勿過濾分數, post-mortem-write-raw靜默拒寫invalid-source-未檢回傳值誤報成功-代理訊號非真副作用
